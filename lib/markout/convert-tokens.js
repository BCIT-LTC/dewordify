...
var reportMarkoutErrors = require("./report-token-errors");
...
function applyMarkoutMap($, markoutMap) {
	...
	for (var marker of Object.keys(markoutMap.wrappers)) {
		var startMarker = markoutMap.start + marker;
		var endMarker = markoutMap.end + marker;
		var $start = $(expectedMarkerTags).filterTokens(startMarker);
		var $end = $(expectedMarkerTags).filterTokens(endMarker);
